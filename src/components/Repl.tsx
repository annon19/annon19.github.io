...
export default class Repl extends React.Component<QueryDbProps, QueryDbState> {
  static defaultProps = {
    hideQuery: false,
    execute: false,
    buttonTxt: "run",
  ...
  componentDidMount() {
    if (this.props.execute) {
      ...
      let result = diel.exec(this.props.query)[0];
  ...
  executeQuery() {
    let result = diel.exec(this.state.query)[0];
    this.setState({
      result,
      queried: true,
    });
